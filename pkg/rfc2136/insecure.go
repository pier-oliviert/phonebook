...
func (c *rfc2136DNS) performInsecureDelete(record *phonebook.DNSRecord, zoneName string) error {
	// Ensure that the zoneName ends with a dot
	if zoneName[len(zoneName)-1] != '.' {
		zoneName = zoneName + "."
	}
	...
	// Create the DNS RR (resource record) for the A record deletion
	rr, err := dns.NewRR(fmt.Sprintf("%s %d IN A %s", recordName, c.defaultTTL, record.Spec.Targets[0]))
	if err != nil {
		return fmt.Errorf("PB-RFC2136-#0012: Failed to create DNS RR for delete: %w", err)
	}

	// Add the record deletion to the message
	msg.Remove([]dns.RR{rr})

	// Send the delete request to the RFC2136 server
	client := new(dns.Client)
	serverAddr := fmt.Sprintf("%s:%d", c.server, c.port)
	_, _, err = client.Exchange(msg, serverAddr)
	if err != nil {
		return fmt.Errorf("PB-RFC2136-#0013: Insecure DNS delete failed: %w", err)
	}
